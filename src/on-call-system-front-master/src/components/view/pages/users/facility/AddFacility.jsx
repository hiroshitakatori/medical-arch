...
function AddFacility() {
  ...
  useEffect(() => {
    if (location.state) {
      ...
      form.setFieldsValue({ contractStartDate: dayjs(location.state.contractStartDate, "YYYY年M月D日") });
      // form.setFieldsValue({ allowLogin: location.state.allowLogin });
      if (location.state.allowLogin == true) {
        setAllowLogin(true)
      }
  ...
  const addfacility = (values) => {
    ...
    values.contract = contract ? contract : "";
    ...
    form.setFieldsValue(contract, values.contract)
    values.allowLogin = allowLogin;
    
    values.managerName = Adminname;
    // if(!values?.contract && values?.contract==""){

    //   form.setFields([
    //     {
    //       name: "contract",
    //       errors: [lang == "en" ? "Contract file is required" : "契約ファイルが必要です"]
    //     }
    //   ]);
  ...
  var userExist = (e) => {
    ...
    http.get(process.env.REACT_APP_BASE_URL + `/admin/facilityList?search=${value}&type=${type}`, [], {
      ...
      .then((res) => {
        console.log(240,res?.data?.message)
        if (res?.data?.message) {
          form.setFields([
            {
              name: e?.target?.id,
            ...
            <Row gutter={16}>
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
                <Form.Item label={t('Edit_Facility.FacilityName')} name="name" rules={[
                  ...
                  {
                    min: 1,
                    message: lang === "en" ? t('Name must be 1-50 character') : "名前は 1 ～ 100 文字にする必要があります",
                  },
                  {
                    max: 100,
                    message: lang === "en" ? t('Name must be 1-50 character') : "名前は 1 ～ 100 文字にする必要があります",
              ...
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
                <Form.Item label={t('Edit_Facility.Phone')} name="mobile" rules={[
                  {
                    required: true,
                    message: lang === "en" ? 'Mobile Number Is Required' : "電話番号は必須です",
                  },
                  {
                    min: 9,
                    message: lang === "en" ? 'Mobile Number must be 9-12 digit' : "携帯電話番号は9～12桁でなければなりません",
                  },
                  {
                    max: 13,
                    message: lang === "en" ? 'Mobile Number must be 9-12 digit!' : "電話番号は 9 ～ 12 桁である必要があります",
                  },
                  {
                  ...
                  <Input prefix="+81" defaultValue={data?.mobile} placeholder={t('Edit_Facility.Phone')} onChange={userExist} />
                </Form.Item>
              </Col>
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
              ...
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
                <Form.Item label={t('Edit_Facility.Facility_Manager_Name')} name="facilityManagerName" rules={[
                  ...
                  {
                    min: 1,
                    message: lang === "en" ? "Facility Manager Name must be 1-100 character" : "施設管理者名は 1 ～ 100 文字である必要があります",
                  },
                  {
                    max: 100,
                    message: lang === "en" ? t('Facility Manager Name must be 1-100 character') : "施設管理者名は 1 ～ 100 文字である必要があります",
                  },
                ]}>
                  <Input defaultValue={data?.facilityManagerName} placeholder={t('Edit_Facility.ManagerName')} />
                </Form.Item>
              </Col>
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
              ...
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
                <Form.Item label={t('Edit_Facility.Capacity')} name="capacity" rules={[
                  ...
                  {
                    max: 12,
                    message: lang === "en" ? t('Max Capacity be 12 digit!') : "最大容量は 12 桁です!",
                  },
                ]}>
              ...
              <Col span={24}>
                <Form.Item label={t('Edit_Facility.Contract_File')} name="contract"

                  rules={[
                    // {
                    //   required: true,
                    //   message: lang === "en" ? 'Contract Is Required' : "契約が必要です",
                    // },
                  ]}
                >
              ...
              <Col sm={{ span: 12 }} xs={{ span: 24 }}>
                ...
                //     {
                //       required: true,
                //       message: lang === "en" ? t('Allow Login Is Required') : "契約開始日は必須です",
                //     },
                //   ]}
                >
              ...
              <ConfigProvider locale={jaJP}>
                ...
                <DatePicker
                    className='custom-ant-picker' placeholder={t('Edit_Facility.Contract_Start_Date')} suffixIcon={<Calendar size="22" color="#707070" />} format={"YYYY年M月D日"}/>
                </Form.Item>
                </ConfigProvider>
